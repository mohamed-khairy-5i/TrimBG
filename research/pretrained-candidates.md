...
اختُبرت نسختان من U2NetP على 100 صورة AIM-500 validation باستخدام نفس preprocessing الرسمي. النسخة الأصلية FP32 حجمها 4,574,861 بايت، ومتوسطها 0.1842 ثانية وMAE = 0.050753. الضغط الديناميكي INT8 خفّض الحجم إلى 1,321,671 بايت، لكنه رفع MAE إلى 0.295832 وأصبح أبطأ على CPU بمتوسط 1.2570 ثانية؛ لذلك رُفض كنسخة إنتاجية. التحويل إلى FP16 خفّض الحجم إلى 2,355,542 بايت، وحافظ على الجودة تقريباً عند MAE = 0.050497 مع متوسط 0.1785 ثانية محلياً؛ وهو ضغط آمن نسبياً للحجم، لكنه يحتاج benchmark داخل Chromium قبل اعتماده.

## U2NetP FP16 benchmark داخل Chromium

بعد إصلاح صفحة benchmark لتعرض الملف المحمّل فعلياً، تم تحميل `u2netp/u2netp-fp16.onnx` بحجم 2,355,542 bytes (حوالي 2.25 MiB). على Chromium مع `crossOriginIsolated=true` و6 خيوط، كان أول تشغيل: `modelReadyMs=322`، و`inferenceMs=657`، و`totalMs=1080`. النتيجة البصرية على صورة الشخص المرجعية كانت جيدة مع فصل واضح للجسم. في التشغيل الثاني بعد التهيئة: `modelReadyMs=0`، و`inferenceMs=625`، و`totalMs=715`. هذا يجعل FP16 أسرع قليلاً من قياس FP32 السابق (~639 ms warm) مع نصف حجم النموذج تقريباً، مع جودة AIM-500 شبه مطابقة لـ FP32.

في اختبار بصري منفصل على `aim_0406` (شعر بإضاءة خلفية)، كان `inferenceMs=667` وظهر قصّ الشعر والجسم متماسكاً مع احتفاظ جيد بالشعيرات الفاتحة، مع هالة خفيفة متوقعة من دقة الإدخال 320×320. النتيجة مناسبة لوضع سريع، لكنها لا تبرر استبدال ISNet في وضع الجودة.

في اختبار `aim_0448` (أرنب بفرو وخلفية نباتية)، كان `inferenceMs=651`. تم فصل جسم الأرنب بشكل متماسك، مع فقدان/تنعيم بعض الشعيرات الدقيقة وتسرب أخضر محدود قرب الحافة السفلية؛ لذلك يظل مناسباً كخيار سريع عام مع توقع جودة أقل من ISNet في الفرو المعقد.

في اختبار `aim_0462` (أحذية وشرائح ليمون على خلفية صفراء)، كان `inferenceMs=617`. فصل النموذج الأجسام بوضوح وحافظ على الأربطة والحواف الرئيسية، مع بقاء جزء أصفر صغير/هالة حول بعض الليمون؛ الأداء عملي لوضع السرعة، وليس مثالاً على جودة matting عالية الدقة.
