...
const requestedSize = query?.get('size') === '128' ? 128 : 256;
const progressive = query?.get('progressive') === '1';
const MODEL_URL = progressive
  ? requestedSize === 128 ? '/models/v3-128-progressive/lite_matting_v3_128.onnx' : '/models/v3-256-progressive/lite_matting_v3_256.onnx'
  : requestedSize === 128 ? '/models/v3-128/lite_matting_v3_128.onnx' : '/models/v3-256/lite_matting_v3_256.onnx';
...
const INPUT_SIZE = requestedSize;
...
export function preloadV3Matting(): Promise<void> {
...
export async function removeBackgroundWithV3(blob: Blob): Promise<{ blob: Blob; timing: V3Timing }> {
  ...
  try {
    ...
    if (!output || !(output.data instanceof Float32Array)) throw new Error('Unexpected V3 output tensor.');
